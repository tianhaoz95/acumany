...
class Whiteboard extends Component {
  constructor(props) {
    super(props);
    var containerElement = document.getElementsByClassName('workspace-session-tool-container');
    this.state = {
      thickness: 3,
      color: "black",
      visible: false,
      enable: false,
      width: containerElement[0].offsetWidth,
      height: containerElement[0].offsetHeight
    };
  ...
  render() {
    return (
      <div>
        <svg className="whiteboard-svg" width={this.state.width} height={this.state.height} ref={(ref) => this.svgRef = ref}>
          <rect className="whiteboard-background"></rect>
        </svg>
        <div className="whiteboard-button">
          <Popover
            className="whiteboard-popover-inner-content"
            content={
